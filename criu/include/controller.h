...
#define SHM_SIZE 32
#define SHM_KEY1 6483
#define SHM_KEY2 8877
...
int zns_set_zone_change_notification(void);
int zns_get_async_event(void);
int zns_get_log(int fd, void * data, __u64 zid);
int check_completion_queue(void);
unsigned int zns_get_wp(unsigned int zone_number);

struct zone_descriptor *zone_desc_list;
pthread_spinlock_t *zone_desc_lock;
struct zns_info *zns_info;
int shmid1;
int shmid2;
char *shm1, *shm2;

unsigned int meta_zone;
unsigned int page_zone;
