...
/* K-V LIST node's value LIST */
struct cr_zns_zone{
	int zone_count; // number of zones per file
	struct _cr_zns_zone *head;
};

/* K-V LIST node's value LIST node */
struct _cr_zns_zone{
	int zone_num;
	int addr_from; /* start LBA  */
	int size; /* last LBA (Size)  */
	struct _cr_zns_zone *link;
};
/* file to zone mapping K-V LIST node */
struct _cr_zns {
	char *file_name; /* key */
	//int zone_num; 
	struct cr_zns_zone *zone_head; /* value: cr_zns_zone List */
	struct _cr_zns *link;
};

/* file to zone mapping K-V LIST */
struct cr_zns {
	int count;
	struct _cr_zns *head;
};
//extern struct cr_zns file_to_zone_map;
struct cr_zns file_to_zone_map;
...
extern int close_image(struct cr_img *);

extern int cr_zns_zone_delete(struct cr_zns_zone *head, int zone_num);
extern int cr_zns_zone_get(struct cr_zns_zone *head);
extern int cr_zns_zone_set(struct cr_zns_zone *head, int zone_num, int addr_from, int add_to);
extern void cr_zns_zone_init(struct cr_zns_zone *head);
extern void cr_zns_zone_show(struct cr_zns_zone *mapping);
extern void cr_zns_zone_search(struct cr_zns_zone *head);


extern int cr_zns_set(struct cr_zns *head, char *file_name);
extern int cr_zns_add(struct cr_zns *head, char *file_name, int zone_num, struct cr_img *pmi);
extern int cr_zns_delete(struct cr_zns *head, char *file_name);
extern int cr_zns_get(struct cr_zns *head, char *file_name);
extern void cr_zns_init(struct cr_zns *head);
extern void cr_zns_show(struct cr_zns *map);
extern int cr_zns_search(struct cr_zns *head, char *file_name);
extern struct _cr_zns* cr_zns_search_file(struct cr_zns *file_node_head, char *file_name);
extern int cr_zns_file_write_zone(struct _cr_zns *file, int write_zone_num, int from, int write_size);

/*
Through following 3 API, CRIU can checkpoint image files to ZNS SSDs
zone_alloc()
zone_put()
zone_get()
