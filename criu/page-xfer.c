...
static int write_pages_loc(struct page_xfer *xfer, int p, unsigned long len)
{
	...
	ssize_t curr = 0;
	//printf("Z1\n");
	//int size_192 = (len/_192KB) * _192KB;
	//if(len%_192KB)
	//	size_192++;
	//char* buffer = malloc(size_192);
	char* buffer = malloc(len);
	int r_size = 0;
	//zns_format();
	while (1) {
		//printf("write_pages_loc : filename = %s, size = %d\n",xfer->pi->name,len-curr);
		//r_size=read(p,buffer,len-curr);
		r_size=read(p,buffer,len);
		//printf("Read Size : %d\n",r_size);
		//printf("Read Date : %s\n",buffer);
		//ZNS_Write(yejin)
		//zone_write(xfer->pi->name, buffer, len-curr ,1);

		zone_write(xfer->pi->name, buffer + curr, r_size ,1);
		ret = r_size;
		//ret = len-curr;
		//ret = write(img_raw_fd(xfer->pi),buffer,len-curr);
		/*
