#include <stdio.h>
#include <stdlib.h>
#include <limits.h> // valid range of int type
...
#include <fcntl.h> // for files on Unix
...
/* Prior log initilization */
/*
	CRIU preserves all (early) log messages in a buffer.
	In case of error, the content of this buffer needs to be printed out (flushed).
*/
...
int main(int argc, char *argv[], char *envp[])
{
	...
	int state = PARSING_GLOBAL_CONF; // 1: configuration and CLI parsing order
	...
	cr_zns_init(&file_to_zone_map); //file_to_zone_init (yejin)
	cr_pb_init(); // protobuf_init
	...
	/* cr_options.h: struct cr_options opts; */
	/* struct cr_options { 
		... 
		enum criu_mode mode; // shows the mode criu is running at the moment: dump/restore/... 
		...};
		-----
	   
	   enum criu_mode{ CR_UNSET=0, CR_DUMP, CR_PRE_DUMP, ...};	
	*/
	if (opts.mode == CR_DUMP) {
		// when checkpointing (dump job) yejin
		if (!opts.tree_id)
			goto opt_pid_missing;
		// go to dump function with pid
		/* function primitives: init cr_dump_tasks(pid_t pid);  */
